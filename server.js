...
async function start() {
  ...
  sock = makeWASocket({ version, auth: state, logger: pino({ level: 'silent' }), browser: ['Ubuntu','Chrome','124'] })
  sock.ev.on('creds.update', saveCreds)

  sock.ev.on('connection.update', ({connection,lastDisconnect}) => {
    console.log("Connection:", connection)
    if (connection==='close' && new Boom(lastDisconnect?.error)?.output?.statusCode!== DisconnectReason.loggedOut) {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (from === META_AI) {
      ...
      // 2) WhatsApp user ko forward (purana flow)
      ...
      if (target) {
        if (m.message.imageMessage) {
          ...
          await sock.sendMessage(target, { text: m.message.conversation || "..." })
        }
