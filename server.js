...
async function start() {
  ...
  sock = makeWASocket({
    ...
    markOnlineOnConnect: true, // ✅ ADDED
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (status!== DisconnectReason.loggedOut) setTimeout(start, 5000)
    }
  })
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // ✅ ADDED: har message log karo
    console.log("IN:", from, "fromMe:", m.key.fromMe, "text:", text.slice(0,30))
      ...
      if (waiting.length > 0) {
        ...
        try {
          if (m.message.imageMessage) {
            ...
            return res.json({ type: "image", mime: "image/jpeg", caption: m.message.imageMessage.caption || "", data: buf.toString('base64') })
          } else {
